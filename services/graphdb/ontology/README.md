...
The same files are mirrored under `data/ontology/` for local-mode tooling (the Ontology Manager UI), which additionally vendors the generated term index (`term-index.{json,md}` from the ontology repo's `docs/`). Keep all copies in sync when bumping the version — see the platform README's *Ontology* section for the refresh recipe.
