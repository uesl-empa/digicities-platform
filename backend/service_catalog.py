...
This module is headless: the "active workspace storage" default is supplied by
whoever hosts it. The Streamlit shim (``components.service_catalog``) registers
a provider that reads the workspace context from session state via
``set_storage_provider``; headless callers pass ``storage=`` explicitly.
"""
...
# Zero-arg callable returning the active workspace's storage (or None). The
# Streamlit shim registers one that reads session state; headless callers
# either register their own or pass ``storage=`` to the functions directly.
_storage_provider: Optional[Callable[[], object]] = None


def set_storage_provider(provider: Optional[Callable[[], object]]) -> None:
    """Register where the "active workspace storage" default comes from."""
...
def _global_client():
    try:
        from backend.nextcloud.global_client import get_global_nextcloud_client
